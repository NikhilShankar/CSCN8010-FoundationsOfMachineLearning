...
class NeuralNetworkEvaluatorVGG:

    def __init__(self, datafolder):
        data_folder = pathlib.Path(datafolder)
        self.train_dataset = self.preprocess_dataset(image_dataset_from_directory(
            data_folder / "train",
            image_size=(180, 180),
            batch_size=32))
        self.validation_dataset = self.preprocess_dataset(image_dataset_from_directory(
            data_folder  / "validation",
            image_size=(180, 180),
            batch_size=32))
        self.test_dataset = self.preprocess_dataset(image_dataset_from_directory(
            data_folder  / "test",
            image_size=(180, 180),
            batch_size=32))
        return
